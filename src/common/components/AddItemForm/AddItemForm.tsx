...
export const AddItemForm: React.FC<PropsType> = memo(({ addItem, disabled }) => {
  ...
  const onKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) =>
    event.key === 'Enter' && title.length < 15 && addItemHandler();
  ...
  return (
    <div>
      <TextField
        ...
        disabled={disabled}
      />
      <IconButton disabled={inputButtonDisabling || disabled} onClick={addItemHandler}>
        <AddBox />
      </IconButton>
