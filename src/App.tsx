...
const App = () => {
    ...
    const changeFilter = (value: FilteredType, todoListId: string) => setTodoList(todoList.map(tl => tl.id === todoListId ? {
        ...tl,
        filter: value
    } : tl))

    const removeTodoList = (todoListId: string) => setTodoList(todoList.filter(tl => tl.id !== todoListId))
